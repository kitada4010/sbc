...
def inspect(time_leng, pattern_leng, top_print):
    ...
    for i, name in enumerate(sheet_names1):
        ...
        psth = np.empty((time_leng, int((leng/time_leng)+1)), dtype=np.int)
        for j in range(time_leng) :
            l = 0
            for k in range(j, leng, time_leng) :
                psth[j][l] = sig1[k+j : k+j+time_leng].sum()
                l += 1
            if(len(psth[j]) > pattern_leng) : 
                for k in range(len(psth[j]) - pattern_leng +1) :
                    if (str(psth[j][k : k + pattern_leng]) in pattern_dict1) : 
                        pattern_dict1[str(psth[j][k : k + pattern_leng])] += 1
                    else : 
                        pattern_dict1[str(psth[j][k : k+ pattern_leng])] = 1
    ...
    for i, name in enumerate(sheet_names2):
        ...
        for j in range(time_leng) :
            ...
            if(len(psth[j]) > pattern_leng) : 
                for k in range(len(psth[j] - pattern_leng +1)) :
                    if (str(psth[j][k : k+ pattern_leng]) in pattern_dict2) : 
                        pattern_dict2[str(psth[j][k : k+ pattern_leng])] += 1
                        sum_dict[str(psth[j][k : k+ pattern_leng])] += 1
                    else : 
                        pattern_dict2[str(psth[j][k : k+ pattern_leng])] = 1
                        if (str(psth[j][k : k+ pattern_leng]) in sum_dict) : 
                            sum_dict[str(psth[j][k : k+ pattern_leng])] += 1
                        else :
                            sum_dict[str(psth[j][k : k+ pattern_leng])] = 1
