...
SKIP="1"
TOP="100"
PROG="$HOME/sbc/python/py/pattern/kullback-distribution2.0.py"
...
do
	...
	# kullbackの計算
#<<CONDUCT
        echo $PYTHON $PROG  $LABELDATAPATH/${file%.*}/${indivi}/${indivi%-*}-${file%-*}-${indivi##*H}3.xlsx  $LABELDATAPATH/${file%.*}/${indivi}/${indivi%-*}-${file%-*}-${indivi##*H}2.xlsx $TIME_RANGE_S $TIME_RANGE_E $PATTERN_RANGE_S  $PATTERN_RANGE_E $SKIP $TOP > jikkou.txt
